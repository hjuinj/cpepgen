...
class test_reactions(unittest.TestCase):
    def test_make_peptide_bond(self):
        gly_smile = "NCC(=O)O"
        gly_mol = Chem.MolFromSmiles(gly_smile)
        # peptide Bond
        mols, reaction_smart = reactions.make_peptide_bond(gly_mol, gly_mol)
    ...
    def test_make_ester_bond(self):
        ...
        # ester Bond
        mols, reaction_smart = reactions.make_ester_bond(gly_mol, alcohol_mol)
        self.assertEqual(Chem.MolToSmiles(mols[0], True), "CCOC(=O)CN")
        self.assertEqual(Chem.MolToSmiles(mols[1], True), "O")

    def test_make_sulfur_bond(self):
        cys_smile = "NC(CS)C(=O)O"
        cys_mol = Chem.MolFromSmiles(cys_smile)

        #sulfur Bond
        mols, reaction_smart = reactions.make_sulfur_bond(cys_mol, cys_mol)
